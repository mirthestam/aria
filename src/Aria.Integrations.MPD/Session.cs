...
public sealed class Session
{
    ...
    public async Task<ConnectionScope> CreateConnectionScopeAsync(CancellationToken token = default)
    {
        ...
        var wrapper = await _connectionPool.GetObjectAsync(token);        
    ...
    {
        using var scope = await CreateConnectionScopeAsync();
        return await scope.SendCommandAsync(command);        
    }
    ...
    private async Task Connect(CancellationToken cancellationToken)
    {
        ...
        StatusConnection = await OpenConnectionAsync(cancellationToken);
    ...
    private async Task Disconnect()
    {
        ...
        if (StatusConnection != null) await StatusConnection.DisconnectAsync();
        ...
        async Task? StatusLoop()
        {
            while (true)
            {
                if (_statusUpdater?.Enabled != true && StatusConnection is { IsConnected: true })
                {
                    ...
                    _statusUpdater.Elapsed += async (s, e) => await UpdateStatusAsync(ConnectionType.Status);
                ...
                try
                {
                    ...
                    var idleChangesTask = Task.Run(async () =>
                        await _idleConnection.SendAsync(
                            new IdleCommand("stored_playlist playlist player mixer output options update")));
